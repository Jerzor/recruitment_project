...
const countDeltas = (symbolKlinesWithDelta) => {
...
const getKlineColor = (kline) => {
  console.log(kline);
  // kolor mówiacy o tym, czy swieca zamkneła sie ze wzrostem, spadkiem czy bez zmain
  // mozna tez po delcie sprawdzać

  const openPrice = parseFloat(kline.openPrice);
  const closePrice = parseFloat(kline.closePrice);

  let color;

  if (closePrice === openPrice) color = Colors.Neutral;
  else if (closePrice > openPrice) color = Colors.Green;
  else if (closePrice < openPrice) color = Colors.Red;
...
const getFormatteData = (symbolKlines) => {
  return symbolKlines.map((kline) => {
    const delta = klinesDiff(
      parseFloat(kline.openPrice),
      parseFloat(kline.closePrice),
    );

    const klineColor = getKlineColor(kline);
...
const start = async () => {
  ...
  const formattedData = getFormatteData(symbolKlines);

  // wykaz wzrostu/spadku ceny w danej swiecy
  const symbolKlinesWithDelta = sortBy(formattedData, "openTime");
  ...
  const { deltaMax, deltaMin } = countDeltas(symbolKlinesWithDelta);
